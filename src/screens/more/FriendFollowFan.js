...
const SectionTitle = ['好友', '关注', '粉丝'];
// 上一个tab下标
let prevTabIndex = 0;
let sectionTitleIndex = 0;

const FriendFollowFan = () => {
  ...
  const [nextTabIndex, setNextTabIndex] = useState(index);
  
  useEffect(() => {
		navigation.setOptions({
			title: SectionTitle[nextTabIndex]
		});
	}, [nextTabIndex]);
  ...
  const [{ }, request0] = useAxios(
		{
			method: 'GET',
		},
		{ manual: true }
  );
  const [{ }, request1] = useAxios(
		{
			method: 'GET',
		},
		{ manual: true }
  ...
  useEffect(() => {
    ...
    const showFriends = async () => {
      const { data } = await request0({
        url: requestApi.getMyFriends
      })
      console.log('+++FriendFollowFan showFriends', data)
      setFriends(data)
    }
    ...
    const showFollows = async () => {
      ...
      console.log('+++FriendFollowFan showFollows', data)
      setFollows(data)
    }
    ...
    const showFans = async () => {
      ...
      console.log('+++FriendFollowFan showFans', data)
      setFans(data)
    }
  ...
  // console.log('---FriendFollowFan', friends, follows, fans)
	return (
          ...
          >
			...
			>
				<View style={[styles.swiper, { width: svContentContainerWidth }]}>
					<PersonList personList={friends}/>
				</View>
				<View style={[styles.swiper, { width: svContentContainerWidth }]}>
					<PersonList personList={follows}/>
				</View>
				<View style={[styles.swiper, { width: svContentContainerWidth }]}>
					<PersonList personList={fans}/>
				</View>
			</ScrollView>
		</SafeAreaView>
	);
...
export default React.memo(FriendFollowFan)