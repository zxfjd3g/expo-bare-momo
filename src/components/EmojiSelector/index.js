...
const TabBar = ({ theme, activeCategory, onPress, width }) => {
	const tabSize = width / categoryKeys.length + 10;
...
const EmojiCell = ({ emoji, colSize, ...other }) => (
	<TouchableOpacity
		...
		{...other}
	>
		<Text style={{ color: '#FFFFFF', fontSize: colSize - 24 }}>{charFromEmojiObject(emoji)}</Text>
	</TouchableOpacity>
);
...
export default class EmojiSelector extends Component {
	...
	handleTabSelect = (category) => {
		if (this.state.isReady) {
			if (this.scrollview) this.scrollview.scrollToOffset({ x: 0, y: 0, animated: false });
			this.setState({
				searchQuery: '',
				category,
			});
